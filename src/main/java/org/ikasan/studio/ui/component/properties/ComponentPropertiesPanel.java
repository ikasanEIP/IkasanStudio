...
public class ComponentPropertiesPanel extends PropertiesPanel {
    ...
    private boolean updateUserCodeManually;
    ...
    protected void doOKAction() {
        ...
        List<String> changedAffectingProperties = getSelectedComponent() instanceof Flow
                ? List.of() : getChangedAffectsUserImplementedClassPropertyLabels();
        ...
        if (dataHasChangedAndOKToProcess()) {
            ...
            populatePropertiesEditorPanel();
            redrawPanel();
            if (getSelectedComponent() instanceof FlowUserImplementedElement implementation) {
                implementation.setOverwriteEnabled(!updateUserCodeManually && !changedAffectingProperties.isEmpty());
                implementation.setUserClassGenerationDeferred(updateUserCodeManually);
            }
    ...
    private boolean confirmComponentSelfEdit(List<String> changedPropertyLabels) {
        ...
        if (affected.isEmpty()) {
            ...
            int choice = Messages.showYesNoCancelDialog(project, message,
                    StudioBundle.message("dialog.ConfirmRegenerateUserImplementedClass"),
                    StudioBundle.message("button.UpdateUserCodeMyself"),
                    StudioBundle.message("button.RegenerateClass"), StudioBundle.message("button.Cancel"), Messages.getWarningIcon());
            updateUserCodeManually = choice == Messages.YES;
        ...
        String message = StudioBundle.message("message.ConfirmRegenerateUserImplementedClassNamed",
                String.join(", ", changedPropertyLabels));
        return confirmWithOptionalBackup(message, affected, true);
    }
    ...
    private boolean confirmWithOptionalBackup(String message, List<AffectedUserImplementedClass> affected) {
        return confirmWithOptionalBackup(message, affected, false);
    }

    private boolean confirmWithOptionalBackup(String message, List<AffectedUserImplementedClass> affected, boolean allowManualUpdate) {
        var dialog = new RegenerateUserClassDialog(project, message,
                affected.stream().map(AffectedUserImplementedClass::description).toList(), allowManualUpdate);
        boolean confirmed = dialog.showAndGet();
        if (confirmed && dialog.isManualUpdateSelected()) {
            updateUserCodeManually = true;
            return true;
        }
