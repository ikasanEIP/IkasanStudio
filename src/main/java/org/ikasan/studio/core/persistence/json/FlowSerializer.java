...
public class FlowSerializer extends StdSerializer<Flow> {
    ...
    protected void serializePayload(Flow flow, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        ...
        if (    flow.getFlowRoute().getFlowElements() != null &&
            ...
            processFlowRouteFlowElements(List.of(flow.getFlowRoute()), jsonGenerator, flowElementSerializer, serializerProvider);
...
//    }
     ...
     * an endpoint or debug.
     * @param flowRoutes to be persisted
     * @param jsonGenerator fasterxml engine that does the persistance
     * @param flowElementSerializer we delegate the flow element serialisation to this
     * @param serializerProvider is needed to accees non-bespoke serialiser
     * @throws IOException if we could not write to file system
     */
