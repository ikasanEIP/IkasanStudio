 ...
 * {@code GeneratedProjectSynchronizer}'s {@code isDebug()} gate), that regeneration would silently produce a blank stub,
...
public final class UserImplementedClassRelocator {
    ...
    public static void relocateIfNeeded(Project project, Module module, FlowElement movedElement, Flow oldFlow, Flow newFlow) {
        ...
        VirtualFile oldFile = StudioProjectFiles.getUserImplementedClassFile(project, oldPackageName, oldClassName);
        if (oldFile == null) {
        ...
        if (movedElement.getComponentMeta().isDebug()) {
            ...
            // (GeneratedProjectSynchronizer.generateAndSaveUserImplementClassStubsForFlow's
            ...
            StudioProjectFiles.deleteFile(project, oldFile);
            movedElement.setPropertyValue(USER_IMPLEMENTED_CLASS_NAME, newClassName);
            return;
        }
