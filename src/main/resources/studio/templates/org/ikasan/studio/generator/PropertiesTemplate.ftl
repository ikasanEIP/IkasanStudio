...
<#if module.properties.ApplicationPackageName.value??>
module.package=${module.properties.ApplicationPackageName.value}
</#if>
...
server.servlet.context-path=/${module.getName()}
...
<#list module.getFlows()![] as flow>
    <#list flow.getFlowComponentList()![] as ikasanComponent>
        <#if (ikasanComponent.getStandardProperties())??>
            <#list ikasanComponent.getStandardProperties() as propName, propValue>
                <#if propValue.getMeta().getPropertyConfigFileLabel() != "" && propValue.getValue()?? && ! propValue.getMeta().getUserImplementedClass()>
