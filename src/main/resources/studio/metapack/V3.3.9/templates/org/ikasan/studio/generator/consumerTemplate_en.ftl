...
@org.springframework.beans.factory.annotation.Value("${r"${"}studio.sample-consumer.${studioPackageTag}.${flowElement.getPropertyValue("userImplementedClassName")}.initial-delay-ms:60000}")
private long sampleInitialDelayMillis = 60000;
@org.springframework.beans.factory.annotation.Value("${r"${"}studio.sample-consumer.${studioPackageTag}.${flowElement.getPropertyValue("userImplementedClassName")}.poll-delay-ms:60000}")
...
@org.springframework.beans.factory.annotation.Value("${r"${"}studio.sample-consumer.${studioPackageTag}.${flowElement.getPropertyValue("userImplementedClassName")}.fixture-input-enabled:false}")
...
Object event = eventFactory.newEvent(identifier, "Hello from ${flowElement.getPropertyValue("userImplementedClassName")}, event #" + sequence);
dispatch(event);
}
