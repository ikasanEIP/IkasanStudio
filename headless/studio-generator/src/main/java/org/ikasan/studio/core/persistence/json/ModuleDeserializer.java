...
import static org.ikasan.studio.core.persistence.json.SerializerUtils.getTypedValue;
import static org.ikasan.studio.core.metapack.model.ComponentPropertyMeta.*;
import static org.ikasan.studio.core.metapack.ComponentLibrary.DEFAULT_IKASAN_PACK;
...
public class ModuleDeserializer extends StdDeserializer<Module> {
    ...
    /**
     * Deserialize the module from the JSON parser, this includes all recursive elements such as flows, flow elements, transitions, etc.
     * @param jp the JsonParser to read from
     * @param ctxt the DeserializationContext
     * @return a Module object
     * @throws IOException if there are issues reading the JSON
     * @throws StudioBuildRuntimeException if there are issues with the build
     */
    @Override
    ...
    protected FlowRoute orderFlowElementsByTransitions(String metapackVersion, List<Transition> transitions, Flow flow, Map<String, FlowElement> flowElementsMap) throws StudioBuildException {
        ...
        if (!transitions.isEmpty()) {
            ...
            if (startKey.isBlank()) {                LOG.warn("STUDIO: ERROR: Could not find the start of the transition chain " + transitions);
            } else {
    ...
    private FlowElement getRouterEndPoint(String metapackVersion, Flow flow, FlowElement router, FlowRoute containingFlowRoute, String routeName) {
        ...
        if (endpointComponentName != null) {
            try {
                ...
                ComponentMeta endpointComponentMeta = ComponentLibrary.getIkasanComponentByKeyMandatory(metapackVersion, endpointComponentName);
                ...
                if (actionProperties != null) {
                        ...
                        while (fieldNames.hasNext()) {
                            ...
                            if (componentPropertyMeta == null) {
                                LOG.warn("STUDIO: DATA CORRUPTION : Attempting to set properties field that has no recorded meta, fieldName=[" + fieldName + "], skipping");
                            } else {
    ...
    public FlowElement getInitialFlowElement(JsonNode jsonNode, Flow containingFlow, String metapackVersion) throws StudioBuildException {
        ...
        if(jsonNode.isObject() && !jsonNode.isEmpty()) {
            ...
            ComponentMeta componentMeta = ComponentLibrary.getIkasanComponentByDeserialisationKey(
                    metapackVersion, implementingClass, componentType, additionalKey);
            if (componentMeta == null)
                LOG.warn("Studio: Serious: Could not create a flow element using implementingClass [" + implementingClass + "] componentType [" + componentType + "] additionalKey [" + additionalKey + "] ignoring element.");
            else {
                ...
                if (flowElement.getComponentMeta().isGeneric()) {
                    flowElement.setPropertyValue("userImplementedClassName", ComponentLibrary.getClassFromSpringString(implementingClass));
                }
