...
* "iconResourceDirectory" - the platform-neutral resource location resolved by the UI
...
Within that folder are the roots of each metapack version e.g. V3.3.9 is the metapack for Ikasan version 3.3.9, V4.1.6 is the metapack for Ikasan version 4.1.6, etc.
...
ComponentLibrary is a singleton that is used to expose metapacks for use within the IDE e.g. by the:
