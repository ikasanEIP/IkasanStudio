...
The ikasan-studio-project-archetype is now available on Maven Central which means you can easily create a Studio project from the Intellij new project wizard. Ensure you select 'Maven Archetype' and a 'Catalog' or 'Maven Central', then start to type 'ikasan-studio', that should be enough for auto-complete to display the full plugin name.
...
In most cases, the wizard will be available without any special setup, if you can not see the wizard you may be subject to a company or local Maven refresh policy, to force a refresh of the archetypes cache, use the maven command line from within any Maven project and issue the following maven command:
