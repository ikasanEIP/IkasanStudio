...
Select a generic **Converter** on the canvas, or drop a new one, and use **Conversion recipe** in its properties. Matching recipes are marked **suggested** using the inferred input and the downstream component's declared input types. For a new converter, the top-ranked suggestion is preselected in the draft as soon as at least one exists - including when several are suggested (e.g. email body versus attachment) - rather than leaving a freshly dropped Converter generating nothing until its properties are opened; any suggestion remains one click away to change. Existing saved recipes and custom converters are preserved, and an existing selection is never overridden. The selector stays available after creation. Selecting a recipe sets the source and target types; it does not save the model or replace Java code. The explanatory text remains visible below the recipe settings.
...
Attaching a file to an email needs two independent things to be true: the Converter must build the attachment (`EmailPayload.addAttachment`), and the Email Producer's own **hasAttachments** property must be set to `true` - the producer ignores any attachment on the payload while that property is left at its default `false`, with no error raised anywhere. Because of that, the generated attachment call is emitted **commented out**, prefixed with a warning naming the property to enable, and the accompanying default email body only names the file (e.g. `Source file: report.csv`) rather than claiming one is attached. A body promising an attachment that the producer is silently dropping would send a developer chasing a bug that isn't there. Uncomment the call and enable `hasAttachments` together to actually send the file; a custom **Attachment email body** is left untouched either way.
